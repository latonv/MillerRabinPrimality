...
function testPrimality(n, { numRounds=undefined, findDivisor=true } = {}) {
  return new Promise((resolve, reject) => {
    try {
      ...
      const nBits = mont.bitLength(n);
      const nSub = n - ONE;

      const r = mont.twoMultiplicity(nSub, nBits); // Multiplicity of prime factor 2 in the prime factorization of n-1
      ...
      const reductionContext = mont.reductionContextFor(n);
      const oneReduced = mont.reduce(ONE, reductionContext); // The number 1 in the reduction context
      const nSubReduced = mont.reduce(nSub, reductionContext); // The number n-1 in the reduction context
      ...
      for (let round = 0; round < numRounds; round++) {
        ...
        const baseReduced = mont.reduce(base, reductionContext);
        let x = mont.pow(baseReduced, d, reductionContext);
        if (x === oneReduced || x === nSubReduced) continue; // The test passed: base^d = +/-1 (mod n)

        // Perform the actual Miller-Rabin loop
        let i;
        for (i = ZERO; i < r; i++) {
          x = mont.sqr(x, reductionContext);
          if (x === oneReduced) {
            ...
            if (findDivisor) {
              divisor = mont.gcd(mont.invReduce(x, reductionContext) - ONE, n);
